...
class SwampRegistry {
     ...
     */
    get(modelName, defaults) {
        if (!this.builtModels.hasOwnProperty(modelName)) {
            this.buildModel(modelName);
        }

        return this.getModelWithDefaults(modelName, defaults);
    }
     ...
     * @return {Object}           A swagger model.
     */
    getModelWithDefaults(modelName, defaults) {
    ...
    buildModel(modelName) {
        ...
        if (!self.api.resolved.definitions.hasOwnProperty(modelName)) {
            throw new TypeError("Swagger Model: " + modelName + " is not defined.");
        }

        let model = {};
        let keys = lodash.keys(this.api.resolved.definitions[modelName].properties);
        ...
        keys.forEach(function (key) {
            ...
            } else if (self.api.resolved.definitions[modelName].hasOwnProperty("properties") &&
                self.api.resolved.definitions[modelName].properties[key].hasOwnProperty("type")) {
                var modelType = self.api.resolved.definitions[modelName].properties[key].type;
